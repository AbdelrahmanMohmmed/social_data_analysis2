...
parser = argparse.ArgumentParser(description="ML-Based Sentiment Models: SVM, Logistic Regression, Random Forest")
...
if not args.svm and not args.logistic and not args.random_forest:
    parser.error("You must enable at least 1 model (--svm, --logistic, and/or --random-forest)")
...
if len(features_df) != len(labels_df):
    raise ValueError(
        f"Features row count ({len(features_df)}) does not match labels row count ({len(labels_df)})."
    )
...
if args.logistic:
    ...
    metrics = evaluate_model(y_test, y_pred_lr)
